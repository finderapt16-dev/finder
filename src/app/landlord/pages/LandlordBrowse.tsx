...
export function LandlordBrowse() {
  ...
  const SidebarContent = () => <LandlordSidebar user={user} verified={user?.isVerified} activeSection="market" unreadNotifications={unreadNotifications} onSectionChange={(section) => navigate(`/dashboard?section=${section}`)} onClose={() => setSidebarOpen(false)} onLogout={() => { logout?.(); navigate("/"); }} />;
  ...
  const PropertyCard = ({ apartment, compact = false }: { apartment: Apartment; compact?: boolean }) => {
    ...
    return (
      <motion.article whileHover={{ y: -3 }} onClick={() => navigate(`/landlord/market/${apartment.id}`)} className="group cursor-pointer overflow-hidden rounded-xl border border-[#E8DED1] bg-white shadow-sm transition-shadow hover:shadow-lg">
