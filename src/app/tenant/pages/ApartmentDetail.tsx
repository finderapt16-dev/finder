...
const STATUS_STYLE: Record<string, string> = { available: "bg-emerald-50 text-emerald-700", occupied: "bg-rose-50 text-rose-700", maintenance: "bg-violet-50 text-violet-700" };
...
export function ApartmentDetail() {
  ...
  const submitReport = async () => {
    ...
    if (evidence.length === 0) return void toast.error("Please upload at least one image or evidence file.");
  ...
  const Sidebar = () => {
    ...
    return <TenantSidebar active="apartments" unreadCount={unreadCount} />;
  };
  ...
  return (
    <div className={`app-shell fixed inset-0 z-50 overflow-hidden text-slate-950 ${landlordPortal ? "landlord-shell landlord-property-detail bg-[#FCFAF7]" : "tenant-detail-colors bg-[#f6f7f9]"}`}>
      {renter && <TenantMobileNavigation active="apartments" unreadCount={unreadCount} />}
      ...
      <div className="app-shell-main h-full min-w-0 flex-1 overflow-y-auto overflow-x-hidden pb-28 lg:pb-20"><main className="app-shell-content app-shell-content-mobile-nav w-full max-w-full px-4 sm:px-6 lg:px-8"><div className="mx-auto w-full max-w-[1380px] min-w-0">
        ...
        <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(280px,.75fr)]"><div className="min-w-0 space-y-5">
          ...
          <section className="min-w-0 rounded-lg border bg-white p-5 shadow-sm"><h2 className="text-lg font-black">Landlord Information</h2><div className="mt-4 flex min-w-0 items-center gap-3 rounded-lg bg-slate-50 p-4"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-full font-black ${landlordPortal ? "bg-[#F3EFEA] text-[#8B735B]" : "bg-[#F3EFEA] text-[#5F5145]"}`}>{landlordName === "Not provided" ? "L" : landlordName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2)}</span><div className="min-w-0"><strong className="block break-words">{landlordName}</strong>{verified && <VerifiedBadge label={landlordPortal ? "Verified Landlord" : "Verified Listing"} className="mt-1" />}</div></div><div className="mt-4 space-y-2 text-sm text-slate-600"><p className="flex min-w-0 gap-2"><Mail className="h-4 w-4 shrink-0" /><span className="min-w-0 break-words">{landlord?.email || "Email not provided"}</span></p><p className="flex min-w-0 gap-2"><Phone className="h-4 w-4 shrink-0" /><span className="min-w-0 break-words">{landlord?.mobile || landlord?.mobileNumber || "Phone not provided"}</span></p></div></section>
      ...
      <div className={`fixed inset-x-0 bottom-0 z-30 border-t bg-white/95 shadow-[0_-8px_30px_rgba(15,23,42,.08)] backdrop-blur ${landlordPortal ? "lg:left-60" : "lg:left-64"}`}><div className="mx-auto flex max-w-[1380px] items-center gap-3 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:px-6 lg:px-8"><div className="mr-auto min-w-0"><strong className={`block break-words text-sm font-black ${landlordPortal ? "text-[#302820]" : "text-[#756A60]"}`}>Prices are listed per room</strong><span className="text-xs font-bold text-emerald-600">{STATUS_LABEL[status]}</span></div>{tenantAccount && <Button variant="outline" onClick={() => void toggleFavorite(apartment.id)} className="shrink-0"><Heart className={`mr-0 h-4 w-4 sm:mr-2 ${favorite ? "fill-rose-500 text-rose-500" : ""}`} /><span className="hidden sm:inline">{favorite ? "Saved" : "Add to Favorites"}</span></Button>}</div></div>
      </div>
      </div>
