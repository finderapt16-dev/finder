import {
  ...
  HelpCircle, Home, LogOut, Mail, MapPin, Menu,
  Phone,
  Plus,
...
export function ApartmentDetail() {
  ...
  const sidebarItems = [{ label: "Apartments", icon: Search, href: "/browse" }, { label: "Favorites", icon: Heart, href: "/favorites" }, { label: "Settings", icon: Settings, href: "/settings" }];
  const Sidebar = () => {
    if (user?.role === "landlord") {
      return <LandlordSidebar user={user} verified={verified} activeSection="overview" onSectionChange={(section) => navigate(`/dashboard?section=${section}`)} onClose={() => setMobileNav(false)} onLogout={() => { logout(); navigate("/"); }} />;
    }
    if (user?.role !== "landlord") return <TenantSidebar active="apartments" />;
    if (user?.role !== "landlord") {
  ...
  return (
    <div className={`app-shell fixed inset-0 z-50 overflow-hidden text-slate-950 ${ownListing ? "landlord-shell landlord-property-detail bg-[#FCFAF7]" : "tenant-detail-colors bg-[#f6f7f9]"}`}>
      ...
      <div className="app-shell-main h-full min-w-0 flex-1 overflow-y-auto overflow-x-hidden pb-28 lg:pb-20"><main className="app-shell-content app-shell-content-mobile-nav w-full max-w-full px-4 sm:px-6 lg:px-8"><div className="mx-auto w-full max-w-[1380px] min-w-0">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-2">{!renter && <button aria-label="Open navigation" onClick={() => setMobileNav(true)} className="app-sidebar-trigger grid h-10 w-10 shrink-0 place-items-center rounded-lg border bg-white lg:hidden"><Menu className="h-5 w-5" /></button>}<Button variant="ghost" onClick={handleBack} className={`min-w-0 max-w-full justify-start px-2 ${ownListing ? "text-[#756A60] hover:bg-[#F3EFEA] hover:text-[#8B735B]" : ""}`}><ArrowLeft className="mr-2 h-4 w-4 shrink-0" /><span className="truncate">{backLabel ?? (ownListing ? "Back to My Properties" : "Back to Browse")}</span></Button></div><div className="flex flex-wrap gap-2 sm:justify-end"><Button variant="outline" onClick={() => void shareListing()} title="Share listing" className={ownListing ? "shrink-0 border-[#E8DED1] text-[#756A60] hover:bg-[#FAF8F5]" : "shrink-0"}><Share2 className="mr-2 h-4 w-4" />Share</Button>{!ownListing && user?.role !== "admin" && <Button variant="outline" size="icon" onClick={() => void toggleFavorite(apartment.id)} title={favorite ? "Remove favorite" : "Add favorite"} className="shrink-0"><Heart className={`h-5 w-5 ${favorite ? "fill-rose-500 text-rose-500" : ""}`} /></Button>}{canEdit && <Button onClick={() => setEditOpen(true)} className={`shrink-0 ${ownListing ? "bg-[#8B735B] hover:bg-[#756A60]" : "bg-orange-500 hover:bg-orange-600"}`}><Edit3 className="mr-2 h-4 w-4" />Edit Property</Button>}</div></div>

        <header className="mb-5 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-end"><div className="min-w-0"><Badge className={`mb-3 rounded-md ${ownListing ? "bg-[#F3EFEA] text-[#8B735B]" : "bg-orange-50 text-orange-700"}`}>For Rent</Badge><h1 className={`max-w-full break-words text-2xl font-black leading-tight sm:text-4xl ${ownListing ? "text-[#302820]" : "text-slate-950"}`}>{apartment.title || "Untitled apartment"}</h1><div className={`mt-3 flex min-w-0 flex-wrap items-center gap-3 text-sm font-medium ${ownListing ? "text-[#756A60]" : "text-slate-500"}`}><span className="flex min-w-0 items-start gap-1.5 break-words"><MapPin className={`mt-0.5 h-4 w-4 shrink-0 ${ownListing ? "text-[#8B735B]" : "text-orange-500"}`} /><span className="min-w-0 break-words">{locationText}</span></span><Badge className={apartment.isPublished === false ? "bg-[#F3EFEA] text-[#756A60]" : "bg-emerald-50 text-emerald-700"}>{apartment.isPublished === false ? "Unpublished" : "Published"}</Badge>{verified && <VerifiedBadge label="Verified Landlord" />}</div></div><div className={`w-full rounded-lg p-5 ${ownListing ? "border border-[#E8DED1] bg-white text-[#302820] shadow-sm" : "bg-gradient-to-br from-orange-500 to-orange-600 text-white shadow-lg"}`}><p className={`text-xl font-black ${ownListing ? "text-[#8B735B]" : ""}`}>Room Pricing</p><p className={`mt-1 text-sm font-semibold ${ownListing ? "text-[#756A60]" : "text-orange-50"}`}>View each room to see its monthly rent.</p><div className={`mt-4 flex flex-wrap items-center justify-between gap-2 border-t pt-3 text-xs font-bold ${ownListing ? "border-[#E8DED1]" : "border-white/25"}`}><span>{STATUS_LABEL[status]}</span><span className={ownListing && availableRooms > 0 ? "text-emerald-700" : ""}>{availableRooms > 0 ? `${availableRooms} ${availableRooms === 1 ? "room" : "rooms"} available` : "All rooms occupied"}</span></div></div></header>
