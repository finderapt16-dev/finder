import { AlertTriangle, Bell, ChevronRight, Heart, HelpCircle, LogOut, Search, Settings, Sparkles, TrendingUp, type LucideIcon } from "lucide-react";
...
export function TenantSidebar({ active = "apartments", unreadCount = 0 }: { active?: TenantSidebarSection; unreadCount?: number }) {
  const navigate = useNavigate();
  const { user, logout } = useAuth();
  const { favorites } = useFavorites();

  return <aside className="app-sidebar flex h-full w-64 shrink-0 flex-col border-r border-[#e8ded1] bg-white">
    <div className="app-sidebar-brand px-5 pb-5 pt-6"><Link to="/browse" className="flex items-center gap-2.5"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#e8ded1] bg-[#faf8f5]"><AppLogo className="h-9 w-9" /></div><div><span className="text-xl font-bold tracking-tight text-[#302820]">AptFindr</span><p className="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#756a60]">La Paz, Iloilo City</p></div></Link></div>
